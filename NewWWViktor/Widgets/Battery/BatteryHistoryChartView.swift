...
struct BatteryHistoryChartView: View {
    ...
    @State private var tooltipSize = CGSize(width: 140, height: 40)
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            let windowDuration = TimeInterval(max(0, windowMinutes - 1)) * stepSeconds
            ...
            let base = ZStack(alignment: .topLeading) {
                ...
                if let hoverIndex, hoverIndex < visible.count, let sample = visible[hoverIndex] {
                    BatteryHistoryTooltip(sample: sample, valueColor: valueColor)
                        .background(
    ...
    private func barHeight(sample: BatteryProvider.BatteryMinuteSample?, in availableHeight: CGFloat) -> CGFloat {
        guard let sample else {
            return max(4, (availableHeight - 8) * 0.25)
        }
    ...
    private func minuteWindowSamples(from startTime: TimeInterval,
        ...
        for index in 0..<count {
            let time = startTime + (Double(index) * stepSeconds)
...
private struct BatteryTooltipSizeKey: PreferenceKey {
    static var defaultValue = CGSize(width: 140, height: 40)
